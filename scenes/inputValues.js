...
inputValues.enter(async ctx => {
    ...
    if (!list.length)
         await ctx.reply('У вас нет активных категорий. Для создания категории напишите её название в качестве ключа в начале строки. Например: "Вес 88".')
    else {
        let msg = 'Ваши личные категории: <i>'
        // list.forEach((c, i) => {
            msg += list.map(x => x.name).join(', ')//`${i + 1}. ${c.name}, `
        // })
        await ctx.replyWithHTML(msg + '</i>')
    } 
...
inputValues.help( ctx => {
    ctx.replyWithHTML("<i>Правила ввода:</i>\n\n"
        + "<b><u>Без ключевого слова вводятся давление, пульс, температура и общее состояние.</u></b>\n"
        +"\t<b>Давление</b> - <i>два целых числа разделенных пробелом, звездочкой или наклонной чертой;</i>\n"
        +"\t<b>Пульс</b> - <i>целое число, может вводиться после давления (разделитель тот же) или отдельно;</i>\n"
        +"\t<b>Температура</b> - <i>дробное число, разделителем может быть точка или запятая.</i>\n"
        +"\t<b>Общее состояние</b> - <i>от одной до пяти звездочек (*). 1 - совсем плохо, 5 - всё отлично.</i>\n<i>120*78*65\n120 78\n120/78/65\n****</i>\n\n"
        + "<b><u>Что нового в программе</u></b>\n"
        + "Теперь вы можете сами <b>создавать категории</b> для хранения ваших данных. "
        + "Чтобы добавить новую категорию, просто введите её название (например «вес» или «шаги») и значение. Программа сохранит запись автоматически.\n"
        + "Позже вы сможете просматривать свои данные и сохранять их в формате PDF.\n"
        + "По умолчанию каждому пользователю доступно до трёх личных категорий.\n"
        + "Категории могут быть любыми — не обязательно связанными со здоровьем. Попробуйте!\n"
        + "<i>Сатурация 92\nВес 82.5\nШаги 10000\nКак-то так.</i>"
    )
})
//--------------------------------------
...
inputValues.on('text', async ctx => {
    ...
    await ctx.replyWithHTML("<i>Для того, что бы бот понимал смысл введенных данных, следует придерживаться следующих правил:</i>\n\n"
       ...
       + "<b><u>Что нового в программе</u></b>\n"
        + "Теперь вы можете сами создавать категории для хранения ваших данных. "
        + "Чтобы добавить новую категорию, просто введите её название (например «вес» или «шаги») и значение. Программа сохранит запись автоматически.\n"
        + "Позже вы сможете просматривать свои данные и сохранять их в формате PDF.\n"
        + "По умолчанию каждому пользователю доступно до трёх личных категорий.\n"
        + "Категории могут быть любыми — не обязательно связанными со здоровьем. Попробуйте!\n\n"
